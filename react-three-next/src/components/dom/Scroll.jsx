...
export default function Scroll({ children }) {
  ...
  useEffect(() => {
    const scenarioData =
      data.scenarios?.[scenario]?.confidence?.medium?.quantiles?.['5']?.data
    ...
    lenis.on('scroll', ({ progress }) => {
      ...
      while (
        ...
        scenarioData[index + 1].year <= targetYear
      ) {
        index++
      }
  ...
  return (
    <div
      ref={wrapper}
      style={{ position: 'absolute', overflow: 'hidden', width: '100%', height: '100%', top: 0 }}
    >
